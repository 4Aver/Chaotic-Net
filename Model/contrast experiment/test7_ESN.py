...
from 研究生课题.前沿研究.Chaotic_Net.对比实验.Layer.reservoir import Reservoir
...
class ESN(nn.Module):
    """ Applies an Echo State Network to an input sequence.
    Multi-layer Echo State Network is based on paper
...
if __name__ == '__main__':
    ...
    model = ESN(input_size=15,hidden_size=30, output_size=1,num_layers=3)
    out = model(x)
    model.fit()
    print(out.shape)
