...
class wo3_independent_Model(nn.Module):
    ...
    def forward(self, in_x):
        ...
        for _ in range(self.num_layers):
            ...
            mix_x = self.dagcn(x.permute(0, 2, 1, 3)).permute(0, 2, 1, 3)
            x = self.FFN(mix_x + x)
...
if __name__ == "__main__":
    x = torch.randn((128,32,30))        # B,L,D
    model = wo3_independent_Model(time_len=32,M=6,list_input_dims=[5,5,5,5,5,5])
    out, _, gates = model(x)
    print(out.shape)
